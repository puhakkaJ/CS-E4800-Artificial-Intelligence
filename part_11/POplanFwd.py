...
### YOUR CODE HERE
    state = list(bstate)[0]
    acts = state.applActions()
    list_appli = []

    for act in acts:
        for state2 in bstate:
            if act in state2.applActions():
                list_appli = list_appli + [act]
...
### CONSISTING OF THE CURRENT ACTION AND THE SUBPLANS.
            ...
            img_B = Bsucc(bstate, act)
            subplan = constructPlan(Bobserve(img_B, obs), path + [bstate], goalstates)
                
            if subplan is None:
                success = False
                break
            else:
                subplans.append((obs, subplan))
        if success:
