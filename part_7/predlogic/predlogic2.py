...
class NOT:
    ...
    def toRelAlg(self,dbSchema):
        fix = reduce(AND,[ ATOM("U",[Var(x)]) for x in self.subformula.freeVars()])
        print(fix.toRelAlg(dbSchema))
        return RAdifference(fix.toRelAlg(dbSchema),self.subformula.toRelAlg(dbSchema))
...
class FORALL:
    ...
    def toRelAlg(self,dbschema):
        if self.var not in self.subformula.freeVars():
            ...
            fix = RArename({"ELEMENT" : self.var },RAtable("U"))
            print(fix)
            return RAdivision(self.subformula.toRelAlg(dbschema),fix)
