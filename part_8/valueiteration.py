...
def value_of(mdp, s, a, v, gamma):
    ...
    successors = mdp.successor_states(s, a)
    sum = 0.0

    for state in successors:
        sum += (mdp.P(s, a, state) * (mdp.R(s, a, state) + gamma * v[state]))
    ...
    while (repeat):
        ...
        for state in mdp.states():
            ...
            for action in mdp.applicable_actions(state):
               maxs.append(value_of(mdp, state, action, val_n, gamma))

            val_n1[state] = max(maxs)

        for state in mdp.states():
            if abs(val_n1[state] - val_n[state]) > (epsilon * ( 1 - gamma))/(2 * gamma):
                repeat = True            
