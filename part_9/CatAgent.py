...
# My implementation is the alpha-beta search and this performs better than the minmax agent. 
# The alpha-beta Agent beats th Minimax 100 per cent of the time (game runned against Minmax 10 times and alpha-beta won 10 times).
...
class Agent(AgentInterface):
    ...
    def info():
        ...
        return {"agent name": "Kissa Agentti",  # COMPLETE HERE
                "student name": ["Jenni Puhakka"],  # COMPLETE HERE
                "student number": ["779616"]}  # COMPLETE HERE
    ...
    def decide(self, state: Oware, actions: list):
        ...
        for action in actions:
            value = self.min_value(state.successor(action), depth - 1, alpha, beta)
            if value > best_val:
                best_val = value
                best_act = action
