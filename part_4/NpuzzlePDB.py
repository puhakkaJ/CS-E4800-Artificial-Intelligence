...
def makePDBheuristic(goalState,tiles):
    ...
    def lowerBound(state):
        abstract = goalState.abstract(tiles)
        distances = breadthFirstSearch(abstract)
        #print("moi",abstract, goalState, "hei", state.abstract(tiles))
...
def makePDBheuristic2(goalState,tiles1,tiles2):
    def lowerBoundNI(state):
        abstract1 = goalState.abstract(tiles1)
        abstract2 = goalState.abstract(tiles2)
        distances1 = breadthFirstSearch(abstract1)
        distances2 = breadthFirstSearch(abstract2)
        #print("moi",abstract, goalState, "hei", state.abstract(tiles))
        return distances1[state.abstract(tiles1)] + distances2[state.abstract(tiles2)]


    def lowerBoundI(state):
        abstract1 = goalState.abstract(tiles1)
        abstract2 = goalState.abstract(tiles2)
        distances1 = breadthFirstSearch(abstract1)
        distances2 = breadthFirstSearch(abstract2)
    ...
    if not (tiles1.intersection(tiles2)):
        return lowerBoundNI
